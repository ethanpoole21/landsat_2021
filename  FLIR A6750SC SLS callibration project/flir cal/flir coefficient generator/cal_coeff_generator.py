...
parser.add_argument(
    "-t",
    "--target_dir",
    default="/Users/ethanpoole/Desktop/fullcal2421",
...
for i in range(num_folders):
    (globals()['bb_%s' % i]) = []
    (globals()['temp%s' % i]) = (10+i*5)
    print(globals()['temp%s' % i])
    for j in np.arange(7e-6, 14e-6, .01e-6):
        (globals()['bb_%s' % i]).append(globals()['temp%s' % i])#(blackbody((globals()['temp%s' % i]),j))
...
np.savetxt('callibration_coefficients.csv', fit, delimiter=',')
