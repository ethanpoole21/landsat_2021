...
# make empty arrays for each temp
for i in range(num_folders):
    globals()['arr_%s' % i] = np.zeros((h,w),np.float)

# fill empty arrays with images and calculate averages for each temp
for i in range(num_folders):
...
# generate temperature values for spectral range
for i in range(num_folders):
    (globals()['bb_%s' % i]) = []
    (globals()['temp%s' % i]) = 273.15 + (10+i*5)
    for j in np.arange(7e-6, 14e-6, .01e-6):
        (globals()['bb_%s' % i]).append(invblackbody(blackbody((globals()['temp%s' % i]),j),j))
...
for x in range(1): #h*w
    img_values = []
    for i in range(num_folders):
        img_values.append((globals()['flat_average_%s' % i])[x])
    plt.plot(img_values,bb_values)
    plt.show
    current_fit = np.polyfit(bb_values,img_values,1)
    fit.append(current_fit)
    m.append(current_fit[0])
    b.append(current_fit[1])
np.savetxt('callibration_coefficients_temp.csv', fit, delimiter=',')