...
length = len(test_paths)

image = rasterio.open(test_paths[1])
image = image.read(1)
...
ax1.imshow(Z,cmap=plt.get_cmap('turbo'),vmin = np.percentile(Z,2), vmax = np.percentile(Z,98))
...
ax2.imshow(fixed_image,cmap=plt.get_cmap('turbo'),vmin = np.percentile(fixed_image,2), vmax = np.percentile(fixed_image,98))
plt.show()